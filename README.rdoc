...
* <b>Client block</b>: this is done by calling <tt>client.block!</tt> and it revoke any access to any users' resources accessed from the client. This
is done when a client is considered not safe.
* <b>User block a client</b>: this is done by calling <tt>access.block!</tt> and it revoke any access to a specific user's resources from the client.
This is done when a user want to revoke any access to his resources.
* <b>User block an access token</b>: this is done by calling <tt>token.block!</tt> and it revoke any access to a specific token from a client. This
is usually done when a user logout from the client.
