...
Once this is done follows these steps:
  ...
  $ git clone git@github.com:Lelylan/rest-oauth2-server.git</tt> (clone the repository)
  $ bundle install</tt> (installa gems)
  $ mongod</tt> (run MongoDB server)
  $ rake spec</tt> (run the test suite)
  $ rails s</tt> (run the Rest OAuth 2.0 Server)
