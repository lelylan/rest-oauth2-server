...
Add `def admin?; true; end` to app/models/user.rb

  $ rails s

Log in at http://localhost:3000/
All oauth administrations is at: http://localhost:3000/oauth

== Admin user definition

Log in and access the admin dashboard where you will find the following sections.
...
create in the controller pizzas you just add the string "pizzas/create".
...
* {Token}[https://github.com/Lelylan/rest-oauth2-server/blob/master/app/models/oauth/oauth_token.rb]: represents the token used to access user's resources.
...
* {Access}[https://github.com/Lelylan/rest-oauth2-server/blob/master/app/models/oauth/oauth_access.rb]: represents the relation between a client and a user, whenever a user grant an authorization.
* {DailyRequests}[https://github.com/Lelylan/rest-oauth2-server/blob/master/app/models/oauth/oauth_daily_request.rb]: represents a daily request from the client on behalf of a specific user.
