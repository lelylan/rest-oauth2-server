...
Before diving into the code we want to give an overview on how Rest OAuth2 Server supports the 3 authorization flows. If
you feel enough comfortable with then, just skip this chapter. 
...
We are working on a gem and a generator to easily integrate the OAuth server into your project, follow us!
...
To do this we need to follow two simple steps:

* Scope definition (in the admin dashboard)
* Resource protection (inside every controller)

== Scope Definition

The scope definition consist in the creation of associations key/actions. In other words, we associate to a 
key a list of actions that it is allowed to access. To do this you need to log in as admin, open a browser 
and go to the /scopes section. Here you can add the scopes that the client will be able to use.

For example, if we want to define a scope <b>read</b> which allow clients to grant authorization for read-only
actions we need to define a record with an alphanumeric name and a white space list of actions you can access.

* name:   <tt>read</tt>
* values: <tt>pizzas/index pizzas/show pastas/index pastas/show</tt>

The only rule you need to remember here is that the values list need to follow a strict name convention where you
put the "controller name", a "/" and the "action name". If this convention is not followed the access will not be
granted. Note that in this way you can give access to any action, not only the RESTful ones.

To define the <b>all</b> scope the logic is similar. Anyway, instead or rewriting a list with all actions, you can 
reuse the keys of already created scopes. This means that we can reuse the write key, which will automatically add
the access for all read-only actions.

* name:   <tt>all</tt>
* values: <tt>read pizzas/create pizzas/update pizzas/destroy pastas/create pastas/update pastas/destroy</tt>

Look at the {tests}[link:/blob/development/spec/extras/scope_spec.rb] to fully understand all possibilities

== Resource protection

Once scope are defined, you need to add one single line to the controllers you want to protect.
...
This is a simple filter which will authorize the access to all actions on a controller, based on the scope access 
a user has given to a client. Using the <tt>before_filter</tt> options, you can exclude some actions (making them
public) or add any logic you need. For example if the <b>pizza</b> has the index action public to everyone, you 
would add the before filter like this.

   before_filter :oauth_authorized, except: "index"

Last, you can decide to apply the authorization check to all actions that accept the JSON format by uncomment the 
<tt>oauth_authorized</tt> line in the authenticate method in the {ApplicationController}[https://github.com/Lelylan/rest-oauth2-server/blob/development/app/controllers/application_controller.rb]
