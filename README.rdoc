...
For Rest OAuth 2.0 Server to work you must have installed: 
...
* <tt>$ git clone git@github.com:Lelylan/rest-oauth2-server.git</tt> (clone the repository)
* <tt>$ bundle install</tt> (installa gems)
* <tt>$ mongod</tt> (run MongoDB server)
* <tt>$ rake spec</tt> (run the test suite)
* <tt>$ rails s</tt> (run the Rest OAuth 2.0 Server)
