...
* <b>Client block for specific user</b>: this is done by calling <tt>access.block!</tt> and it revoke any access to a specific user's resources.

In both cases, is possible to unblock the client using the <tt>unblock!</tt> method.
