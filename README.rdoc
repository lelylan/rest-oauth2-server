...
To install the project run the following commands from your terminal
...
If everything works fine, you have your OAuth server up and running. We are also working at a gem and a 
...
to access the admin dashboard where you will find the folllowing voices:
...
While <b>users<b> and <b>scopes</b> are visible only to the admin, <b>accesses</b> and <b>Clients</b> will be 
available to every registered user. To better understand what you can do explore the Dashboard and read the 
following sections.
...
* <tt>Name</tt>: unique name that identify a scope.
* <tt>Values</tt>: list of space separated strings, each of one refers to a specific action or to a scope. 

This means that the values can simply be a Scope Name already defined, or a string that identify a specific 
action and that <b>must be followint the convention</b> <tt>controller name</tt> + <tt>/</tt> + <tt>action 
name</tt>
...
<tt>destroy</tt>. Now, lets supppose we want to define the following scopes:

* <b>all</b>: enable the access to all actions.
* <b>read</b>: enable the access to the read-only actions.
* <b>pizzas</b>: enable the access to all pizzas actions.
* <b>pastas</b>: enable the access to all pastas actions.

To do this we need to define four scopes in the admin dashboard, where each of them will define what can or can 
not be accessed.

Pizzas scope  (made up of action name values)
* Name:   <tt>pizzas</tt>
* Values: <tt>pizzas/index pizzas/show pizzas/create pizzas/update pizzas/destroy</tt>

Pastas scope (made up of action name values)
* Name:   <tt>pastas</tt>
* Values: <tt>pastas/index pastas/show pastas/create pastas/update pastas/destroy</tt>
